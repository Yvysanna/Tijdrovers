...
for x in range(500):
    ...
    for student in students_set:
        for course in student.courses:
            all_activities = course._lectures + course._tutorials + course._labs
            activities = [
                activity for activity in all_activities if activity.confirm_registration(student)]
            for activity in activities:
                room, day, time = planner.get_info(activity)
                df_dict['student'].append(
                    student._last_name + ' ' + student._first_name)
    ...
    points.append(ugly_checker.count_points(classrooms_list, course_set))
print(mean(points))
plt.hist(points, range=(1300, 2000), color='midnightblue', edgecolor='mediumblue', density=True, bins=28)
plt.xlim(1300, 2000)
plt.title("Probability distribution of maluspoints across 500 runs")
